...
<div class="note-card-wrapper swipe-row {{ $viewMode === 'list' ? 'note-list-row' : '' }}">
    ...
    <div id="note-card-{{ $note->id }}"
        ...
        {{-- ════ GRID content ════ --}}
            ...
            {{-- Image grid (ẩn nếu note có mật khẩu) --}}
            @if(!$note->has_password && $note->images->count() > 0)
            @php $imgs = $note->images->take(4); $total = $note->images->count(); @endphp
            <div class="note-img-grid note-img-grid--{{ min($total, 4) }} mt-2">
                @foreach($imgs as $i => $image)
                <div class="note-img-cell {{ ($i === 3 && $total > 4) ? 'note-img-cell--more' : '' }}"
                     data-more="{{ $total > 4 ? '+'.($total - 4) : '' }}">
        ...
        {{-- ════ LIST content ════ --}}
            ...
            {{-- Image grid below content (ẩn nếu note có mật khẩu) --}}
            @if(!$note->has_password && $note->images->count() > 0)
            @php $imgs = $note->images->take(4); $total = $note->images->count(); @endphp
            <div class="note-img-grid note-img-grid--{{ min($total, 4) }} mt-2">
                @foreach($imgs as $i => $image)
